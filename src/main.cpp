...
uint256 hashGenesisBlock("0x000005df10c063ad671e3e5a7ca29ba3c4efa8e12a35648e480e53a4e95d54a3");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Joblocks: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Joblocks Signed Message:\n";
...
{
    // Joblocks: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // Joblocks: each block
static const int64 nTargetSpacing = 1 * 60; // Joblocks: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Joblocks: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Joblocks - DarkGravity, written by Evan Duffield - evan@Joblocks.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Joblocks: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Joblocks: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000003f56e4c8a371f8b00edb823003ffca6aa434292ba1223f3389b0953b61");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Joblocks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb5f2c7abac6e2e16a86e557be32d447c5b65df2426f33ffaad82b52a2810b9f6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // Joblocks: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JoblocksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JoblocksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JoblocksMiner(CWallet *pwallet)
{
    printf("JoblocksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Joblocks-miner");
    ...
    try { while (true) {
        ...
        printf("Running JoblocksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JoblocksMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JoblocksMiner, pwallet));
}
